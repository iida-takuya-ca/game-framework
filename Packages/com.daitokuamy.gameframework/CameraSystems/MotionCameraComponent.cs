...
namespace GameFramework.CameraSystems {
    ...
    public class MotionCameraComponent : MotionCameraComponentBase {
        ...
        public void Setup(Context context, int cameraIndex, float time, Vector3 rootPosition, Quaternion rootRotation, LayeredTime layeredTime = null) {
            ...
            // アニメーション部分初期化
            SetupInternal(time, context.clip, rootPosition, rootRotation, layeredTime);
        ...
        /// <param name="rootRotation">ルート向き</param>
        /// <param name="layeredTime">時間軸コントロール用</param>
        public void Setup(AnimationClip animationClip, int cameraIndex, float time, Vector3 rootPosition, Quaternion rootRotation, LayeredTime layeredTime = null) {
            Setup(new Context { clip = animationClip }, cameraIndex, time, rootPosition, rootRotation, layeredTime);
        }
        ...
        /// <param name="rootRotation">ルート向き</param>
        /// <param name="layeredTime">時間軸コントロール用</param>
        public void Setup(AnimationClip animationClip, Vector3 rootPosition, Quaternion rootRotation, LayeredTime layeredTime = null) {
            Setup(new Context{ clip = animationClip }, 0, 0.0f, rootPosition, rootRotation, layeredTime);
        }
