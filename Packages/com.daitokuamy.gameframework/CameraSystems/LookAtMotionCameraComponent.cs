...
namespace GameFramework.CameraSystems {
    ...
    public class LookAtMotionCameraComponent : MotionCameraComponentBase {
        ...
        public void Setup(Context context, int slotIndex, float time, Vector3 rootPosition, Quaternion rootRotation, LayeredTime layeredTime = null) {
        ...
        /// <param name="rootRotation">ルート向き</param>
        /// <param name="layeredTime">時間軸コントロール用</param>
        public void Setup(AnimationClip animationClip, int slotIndex, float time, Vector3 rootPosition, Quaternion rootRotation, LayeredTime layeredTime = null) {
            Setup(new Context { clip = animationClip }, slotIndex, time, rootPosition, rootRotation, layeredTime);
        }
